...
internal sealed class CommunityParticipationOutboxRecordConfiguration
    : IEntityTypeConfiguration<CommunityParticipationOutboxRecord>
{
    public void Configure(
        EntityTypeBuilder<CommunityParticipationOutboxRecord> builder)
    {
        builder.ToTable(
            ...
            tableBuilder =>
            {
                tableBuilder.HasCheckConstraint(
                    "ck_community_outbox_event_version",
                    "\"event_version\" > 0");
                tableBuilder.HasCheckConstraint(
                    "ck_community_outbox_serialized_event",
                    "octet_length(\"serialized_event\") > 0");
                tableBuilder.HasCheckConstraint(
                    "ck_community_outbox_attempt_count",
                    "\"attempt_count\" >= 0");
                tableBuilder.HasCheckConstraint(
                    "ck_community_outbox_claim",
                    "(\"claimed_by\" IS NULL AND " +
                    "\"claim_expires_at_utc\" IS NULL) OR " +
                    "(\"claimed_by\" IS NOT NULL AND " +
                    "\"claim_expires_at_utc\" IS NOT NULL)");
                tableBuilder.HasCheckConstraint(
                    "ck_community_outbox_stalled",
                    "NOT \"is_stalled\" OR " +
        ...
        builder.Property(record => record.EventId)
            .HasColumnName("event_id").HasColumnType("uuid");
        ...
        builder.Property(record => record.EventVersion)
            .HasColumnName("event_version").HasColumnType("integer");
        ...
        builder.Property(record => record.ClaimedBy)
            .HasColumnName("claimed_by").HasColumnType("uuid");
        ...
        builder.HasOne<CommunityParticipationRecord>()
            .WithOne()
            .HasForeignKey<CommunityParticipationOutboxRecord>(
                record => record.CommunityParticipationId)
            .OnDelete(DeleteBehavior.Restrict)
            .IsRequired();
        builder.HasIndex(record => record.CommunityParticipationId)
            .IsUnique()
            .HasDatabaseName(
